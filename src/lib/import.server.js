...
export async function importConversationExport(json) {
    let parsed;
    try {
    ...
    for (let i = 0; i < pairs.length; i += 500) {
        await supabaseAdmin.from("training_pairs").insert(pairs.slice(i, i + 500).map((p) => ({ ...p, source: "upload", status: "approved" })));
    }
