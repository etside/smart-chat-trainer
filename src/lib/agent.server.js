...
const lastLookup = {};
async function getFreshStockData(query) {
    const now = Date.now();
...
export async function findExamples(query, limit = 8) {
...
export async function generateReply(message, history = [], versionId) {
...
export async function logConversation(externalId, source, turns) {
    ...
    if (!conv)
        return;
    await supabaseAdmin.from("messages").insert(turns.map((t, i) => ({
