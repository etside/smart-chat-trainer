...
export const updateSyncSchedule = createServerFn({ method: "POST" })
    .inputValidator((d) => z.object({ schedule: z.enum(["manual", "hourly", "daily", "weekly"]) }).parse(d))
    .middleware([requireSupabaseAuth])
...
export const previewSync = createServerFn({ method: "POST" })
    .inputValidator((d) => z.object({ url: z.string().url().default(DEFAULT_SYNC_URL) }).parse(d || {}))
    .middleware([requireSupabaseAuth])
    .handler(async ({ context, data }) => {
    await assertAdmin(context.supabase, context.userId);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    ...
    try {
        ...
        if (!syncRes.ok)
            throw new Error(`Preview failed: ${syncRes.statusText} (${syncRes.status})`);
...
export const syncCatalog = createServerFn({ method: "POST" })
    ...
    try {
        ...
        const trainingPairs = items
            .filter((item) => (item.name || item.title) && item.price)
            .map((item) => ({
        ...
        if (run) {
            ...
            await triggerTraining({ data: { sync_run_id: run.id } });
            // Log sync usage
