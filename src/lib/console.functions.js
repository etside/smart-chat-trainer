...
export const importPairs = createServerFn({ method: "POST" })
    ...
    await assertAdmin(context.supabase, context.userId);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    await supabaseAdmin.from("training_pairs").insert(data.items.map((i) => ({
...
export const importConversationsJson = createServerFn({ method: "POST" })
    .inputValidator((d) => z.object({ json: z.string().min(2).max(4000000) }).parse(d))
    .middleware([requireSupabaseAuth])
    .handler(async ({ context, data }) => {
    await assertAdmin(context.supabase, context.userId);
...
export const createApiKey = createServerFn({ method: "POST" })
    .inputValidator((d) => z.object({ name: z.string().min(1).max(60), version_id: z.string().uuid().optional() }).parse(d))
    .middleware([requireSupabaseAuth])
    .handler(async ({ context, data }) => {
    await assertAdmin(context.supabase, context.userId);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
...
export const exportTrainingData = createServerFn({ method: "POST" })
    .inputValidator((d) => z.object({ type: z.enum(["training_pairs", "conversations"]) }).parse(d))
    .middleware([requireSupabaseAuth])
    .handler(async ({ context, data }) => {
    await assertAdmin(context.supabase, context.userId);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
...
export const transcribeVoice = createServerFn({ method: "POST" })
    .inputValidator((d) => z
    .object({
    audio: z.string().min(10).max(20000000),
...
export const extractPairsFromText = createServerFn({ method: "POST" })
    .inputValidator((d) => z.object({ text: z.string().min(2).max(12000) }).parse(d))
    .middleware([requireSupabaseAuth])
    .handler(async ({ context, data }) => {
    await assertAdmin(context.supabase, context.userId);
    const raw = await chatComplete([
        {
            role: "system",
            content: 'তুমি একটি ট্রেনিং-ডেটা এক্সট্রাক্টর। ইনপুট টেক্সট (বাংলা/বাংলিশ, ভয়েস ট্রান্সক্রিপ্ট হতে পারে) থেকে কাস্টমার-প্রশ্ন ও দোকানের উত্তরের জোড়া বের করো। শুধু JSON অ্যারে দাও, অন্য কিছু লিখবে না। ফরম্যাট: [{"question":"...","answer":"..."}]. যদি টেক্সটে শুধু তথ্য/নিয়ম থাকে, তাহলে সম্ভাব্য কাস্টমার প্রশ্ন নিজে বানিয়ে সেই তথ্য দিয়ে উত্তর সাজাও। সর্বোচ্চ ১০টি জোড়া।',
        },
        { role: "user", content: data.text },
    ], "openai/gpt-5.6-terra");
    if (typeof raw !== 'string')
        throw new Error("Expected string response from AI");
    ...
    try {
        ...
        if (Array.isArray(parsed)) {
            items = parsed
                .filter((p) => typeof p?.question === "string" &&
...
export const getIngestionStats = createServerFn({ method: "GET" })
    ...
    const [lastSync, deliveryCounts, retryHistory] = await Promise.all([
        ...
        supabaseAdmin.from("webhook_logs").select("processing_status").then(({ data }) => {
            ...
            data?.forEach(log => {
                const status = log.processing_status;
