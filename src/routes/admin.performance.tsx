...
function PerformanceDashboard() {
  ...
  const mutation = useMutation({
    mutationFn: (vars: any) => updateSettings({ data: vars }),
    onSuccess: () => {
  ...
  if (isLoading) return <div className="p-10">লোড হচ্ছে...</div>;

  const metrics = data?.metrics || [];
  const settings = data?.settings || { max_simultaneous_replies: 5, enable_streaming: true };

  const avgTranscription = metrics.filter((m: any) => m.action === 'transcription').reduce((acc: number, m: any) => acc + m.duration_ms, 0) / (metrics.filter((m: any) => m.action === 'transcription').length || 1);
  const avgAnalysis = metrics.filter((m: any) => m.action === 'analysis').reduce((acc: number, m: any) => acc + m.duration_ms, 0) / (metrics.filter((m: any) => m.action === 'analysis').length || 1);
  const avgReply = metrics.filter((m: any) => m.action === 'reply').reduce((acc: number, m: any) => acc + m.duration_ms, 0) / (metrics.filter((m: any) => m.action === 'reply').length || 1);
