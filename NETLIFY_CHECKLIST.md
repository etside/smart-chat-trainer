...
### Preview Deployments
Netlify is configured to create **Deploy Previews** for every GitHub Pull Request. 
- Ensure "Build settings > Deploy previews" is enabled in your Netlify UI.
- Previews will use the `SYNC_TOKEN` and `WEBHOOK_SECRET` from your site environment to validate training-data ingestion changes before merging.

---
**Note:** After adding these variables, trigger a new deploy. The `netlify.toml` in the root handles the build and redirect logic, using the `dist/` directory generated by TanStack Start.
